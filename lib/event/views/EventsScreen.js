...
var EventsScreen = React.createClass({
  ...
  render: function() {
    var content = this.state.dataSource.getRowCount() === 0 ?
      <NoResults
        ...
        /> :
      <RefreshableListView
        loadData={this.forceLoadFestivalEvents}
        refreshDescription="Odświeżam listę wydarzeń"
        ref="listview"
  ...
  handleAdapterData: function(err, events) {
    console.log('handleAdapterData: ', err, events);
    ...
    if (err) {
      ...
      this.setState({
        isLoading: false,
        dataSource: this.getDataSource(events),
        error: null,
      });
    }
  },

  forceLoadFestivalEvents: function() {
    this.timeoutID = null;

    var self = this;
    this.setState({isLoading: true});

    this.adapter.forceRefreshData(this.props.festival, function (err, events){
      return self.handleAdapterData(err, events);
    });
  },

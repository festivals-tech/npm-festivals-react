...
var FestivalsAdapter = function FestivalsAdapter() {
  ...
  var saveFestival = function saveFestival(festival, callback) {
    ...
    database.executeSQL(sql,
      [
        ...
        festival.mainImage ? festival.mainImage.large : null,
        festival.locations.length ? festival.locations[0].name : null,
        festival.locations.length ? festival.locations[0].city : null,
        festival.locations.length ? festival.locations[0].state : null,
        festival.locations.length ? festival.locations[0].country : null,
        festival.locations.length ? festival.locations[0].street : null,
        festival.locations.length ? festival.locations[0].zip : null,
        festival.status,
        festival.type,
        festival.createdAt,
        festival.updatedAt,
  ...
  var getFestivals = function getFestivals(callback) {
    loadFestivals(function (err, festivals) {
      ...
      return forceRefreshData(callback)
    });
  };

  var forceRefreshData = function forceRefreshData(callback) {
  ...
  console.log('CREATE FestivalsAdapter!');
  initTable(function (err) {
